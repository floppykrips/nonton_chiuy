...
async function loadAll() {
  try {
    ...
    allFilms.trending = trending.results || [];
    allFilms.latest   = latest.results   || [];
    allFilms.top      = topRated.results || [];
    allFilms.tv       = tvPop.results    || [];
...
function renderHero(film) {
  ...
  document.getElementById('heroMeta').innerHTML = `
    <span class="star">★ ${film.vote_average?.toFixed(1)}</span>
    <span class="dot"></span>
...
function renderSlider(id, films, badge = false, isNew = false, isTV = false) {
  ...
  (films || []).slice(0, 20).forEach((f, i) => {
    ...
    const rating = f.vote_average?.toFixed(1) || '–';
...
function openModal(film, isTV = false) {
  ...
  const desc   = film.overview || 'Deskripsi tidak tersedia.';
  const rating = film.vote_average?.toFixed(1) || '–';
  const poster = film.poster_path ? `${IMG}${film.poster_path}` : NO_IMG;
  ...
  document.getElementById('modalMeta').innerHTML    = `
    ...
    <span class="tag">${isTV ? 'Serial TV' : 'Film'}</span>
    <span class="tag">TMDb ID: ${film.id}</span>
  `;

  // Tampilkan poster dulu sambil nunggu trailer
